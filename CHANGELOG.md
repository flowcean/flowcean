...
- **Breaking:** Hybrid `Trace` construction requires a `location_time` array, and `Event` construction requires `location_time_before` and `location_time_after`. Tabular trace exports always include the separate `location_time` column. HyDRA simulation also supplies residence times, starting at zero and resetting when the selected mode changes.
