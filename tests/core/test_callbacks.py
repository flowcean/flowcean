...
class TestCreateCallbackManager:
    ...
    def test_create_from_none_returns_empty_manager(self) -> None:
        """Test creating manager from None returns no callbacks."""
        manager = create_callback_manager(None)
        assert isinstance(manager, CallbackManager)
        assert manager.callbacks == []

    def test_learner_without_callbacks_is_silent(
        self,
        capsys: pytest.CaptureFixture[str],
    ) -> None:
        """Test learners stay silent when callbacks are omitted."""
        import numpy as np

        from flowcean.sklearn import RandomForestRegressorLearner

        learner = RandomForestRegressorLearner(n_estimators=10)
        inputs = pl.LazyFrame({"x1": [0.0, 1.0], "x2": [1.0, 0.0]})
        outputs = pl.LazyFrame({"y": np.array([1.0, 0.0])})
...
class TestRichCallback:
    ...
    def test_rich_callback_available_from_callback_package(self) -> None:
        """Test package RichCallback re-exports the public symbol."""
        from flowcean.core.callbacks import RichCallback as PackageRichCallback

        assert PackageRichCallback is RichCallback
...
class TestRiverLearnerValidation:
    """Test River learner input validation."""

    def test_progress_interval_must_be_positive(self) -> None:
        """Test RiverLearner rejects non-positive progress intervals."""
        ...
        with pytest.raises(
            ValueError,
            match="progress_interval must be greater than 0",
        ):
            RiverLearner(
