"""generate_stacked_images_cnn.py

This script processes multiple ROS 2 bag files to generate a labeled image
dataset for CNN-based localization failure detection. It loads map, LiDAR,
pose, and particle cloud data, synchronizes them in time, computes localization
status (localized or delocalized), and converts each sample into a stacked
2D feature image. The resulting images are saved under:
    exported_dataset/localized/
    exported_dataset/delocalized/
"""
...
# Paths
output_root = Path("exported_dataset")
(output_root / "localized").mkdir(parents=True, exist_ok=True)
(output_root / "delocalized").mkdir(parents=True, exist_ok=True)

# Your Polars DataFrame with 5 columns: /map, /scan, /particle_cloud, /amcl_pose, is_delocalized
df = samples_train_lf.collect()

# Create a FeatureImagesData dataset to reuse the same image conversion logic
dataset = FeatureImagesData(
    inputs=df.drop("is_delocalized"),
    outputs=df.select("is_delocalized"),
    image_size=512,
    width_meters=20.0,
)

for idx in range(len(dataset)):
    image_tensor, label_tensor = dataset[idx]  # returns (Tensor, Tensor)
    image = image_tensor.numpy().transpose(1, 2, 0)  # (H,W,C)
    image = (
    ...
    label = bool(label_tensor.item())
    subdir = "delocalized" if label else "localized"
    out_path = output_root / subdir / f"{idx:06d}.png"
