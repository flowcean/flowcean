...
def thermostat_cooling_stream(t: float) -> np.ndarray:
    rate_correction = 0.1*np.sin(0.7 * t)
    return np.array([rate_correction], dtype=float)


def thermostat_delay(
    ambient: float = 20.0,
    ...
    def heating(
        ...
        temperature = state[0]
        cooling_rate_correction = float(input_stream(t)[0])
        return np.array(
            [
    ...
    def event_surface_delay(
        ...
        return state[1] - params["delay_time"]
