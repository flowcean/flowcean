...
def _make_dynamics(dimension: int, matrix: np.ndarray) -> ContinuousDynamics:
    ...
    return ContinuousDynamics(name="", flow=flow)
...
def _build_locations_and_transitions(
    ...
    reset: Reset,
    guard: Guard,
) -> tuple[dict[str, Location], list[Transition]]:
    ...
    return location_map, transitions
...
def mode_cycle(
    ...
    guard = Guard(name="dwell", fn=_guard_clock, direction=1)
    reset = Reset(name="reset_clock", fn=_reset_clock)
    location_map, transitions = _build_locations_and_transitions(
        matrices,
        reset,
        guard,
    )
