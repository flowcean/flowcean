...
def pid_controlled_plant(
    ...
    """
    linear_dynamics = ContinuousDynamics(
        name="linear_dynamics",
        flow=_flow_linear,
    )
    sat_high_dynamics = ContinuousDynamics(
        name="sat_high_dynamics",
        flow=_flow_sat_high,
    )
    sat_low_dynamics = ContinuousDynamics(
        name="sat_low_dynamics",
        flow=_flow_sat_low,
    )
    linear = Location(name="linear", dynamics=linear_dynamics)
    sat_high = Location(name="sat_high", dynamics=sat_high_dynamics)
    sat_low = Location(name="sat_low", dynamics=sat_low_dynamics)

    transitions = [
        Transition(
            source_location="linear",
            target_location="sat_high",
            guard=Guard(
                name="hit_high",
                fn=_guard_high,
                direction=1,
        ...
        Transition(
            source_location="sat_high",
            target_location="linear",
            guard=Guard(
                name="leave_high",
                fn=_guard_high,
                direction=-1,
    ...
    return HybridSystem(
        locations={"linear": linear, "sat_high": sat_high, "sat_low": sat_low},
