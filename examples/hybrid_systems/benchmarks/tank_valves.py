...
def tank_valves(
    ...
    open_dynamics = ContinuousDynamics(name="open_dynamics", flow=flow_open)
    closed_dynamics = ContinuousDynamics(
        name="closed_dynamics",
        flow=flow_closed,
    )
    open_mode = Location(name="open", dynamics=open_dynamics)
    closed_mode = Location(name="closed", dynamics=closed_dynamics)

    transitions = [
        Transition(
            source_location="closed",
            target_location="open",
            guard=Guard(
                name="level_high",
                fn=guard_open,
                direction=1,
        ...
        Transition(
            source_location="open",
            target_location="closed",
            guard=Guard(
                name="level_low",
                fn=guard_close,
                direction=-1,
    ...
    return HybridSystem(
        locations={"open": open_mode, "closed": closed_mode},
        transitions=transitions,
        initial_location="closed",
