...
def time_forced_switch(
    ...
    fast_dynamics = ContinuousDynamics(name="fast_dynamics", flow=flow_fast)
    slow_dynamics = ContinuousDynamics(name="slow_dynamics", flow=flow_slow)
    fast = Location(name="fast", dynamics=fast_dynamics)
    slow = Location(name="slow", dynamics=slow_dynamics)
    guard = Guard(name="dwell", fn=guard_dwell, direction=1)
    reset = Reset(name="reset_clock", fn=reset_clock)

    to_slow = Transition(
        source_location="fast",
        target_location="slow",
        guard=guard,
        reset=reset,
    )
    to_fast = Transition(
        source_location="slow",
        target_location="fast",
        guard=guard,
        reset=reset,
    )
