...
def _feature_columns(config: SelectorFeatureConfig) -> tuple[str, ...]:
    columns = [
        ...
        *config.derivative_columns,
    ]
    columns.extend(
        _history_columns(config.state_columns, config.state_history),
    )
    columns.extend(
        _history_columns(config.input_columns, config.input_history),
    )
    columns.extend(
        _history_columns(config.derivative_columns, config.derivative_history),
    )
    columns.extend(
...
def _project_trace(
    ...
    *,
    ...
    expressions: list[pl.Expr] = [
        *[pl.col(column) for column in config.state_columns],
        *[pl.col(column) for column in config.input_columns],
        *[pl.col(column) for column in config.derivative_columns],
    ]

    expressions.extend(
        _history_expressions(config.state_columns, config.state_history),
    )
    expressions.extend(
        _history_expressions(config.input_columns, config.input_history),
    )
    expressions.extend(
